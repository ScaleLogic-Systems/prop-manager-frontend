...
export const TenantsTab: React.FC<{ propertyId?: string }> = ({ propertyId }) => {
  ...
  const fetchTenants = async () => {
    try {
      ...
      const res = await fetch(`/agent/api/tenants${queryParam}`, { headers, cache: 'no-store' });
      if (res.ok) {
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            Manage active tenants occupying units under your assigned portfolio.
          </p>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        {tenants.length === 0 ? (
          <div className="p-12 text-center text-slate-400 text-sm">
            No active tenants found for this property scope.
          </div>
        ) : (
              ...
              {tenants.map((tenant) => (
                <tr key={tenant.id} className="hover:bg-slate-50/50 transition">
                  ...
                  <td className="p-4">
                    ...
                    <span className="inline-block mt-1 px-2.5 py-0.5 bg-slate-100 text-slate-800 rounded-md text-xs font-bold">
                      Unit {tenant.unit_number}
                    </span>
                  </td>
                  <td className="p-4 space-y-1">
                    <div className="flex items-center gap-2 text-xs text-slate-600">
                      <Mail size={12} className="text-slate-400" /> {tenant.email || 'N/A'}
                    </div>
                    <div className="flex items-center gap-2 text-xs text-slate-600">
                      <Phone size={12} className="text-slate-400" /> {tenant.phone || 'N/A'}
                    </div>
                  </td>
