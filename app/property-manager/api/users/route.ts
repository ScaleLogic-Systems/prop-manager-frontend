...
export async function GET() {
  try {
    ...
    const [{ data: tenants, error: tenantsError }, { data: staff, error: staffError }] = await Promise.all([
      ...
      db.from('properties').select('caretaker_id, caretaker:caretaker_id(id, full_name, email, phone, role)').in('id', propertyIds),
    ]);
    ...
    const users: Array<{ id: string; full_name: string; email: string; phone: string; role: string; property_id: string; property_name: string; unit_number?: string; status: string; invited_at: string }> = (tenants || []).map((tenant) => ({
      ...
      role: 'tenant', property_id: tenant.property_id, property_name: propertyMap.get(tenant.property_id) || '',
      unit_number: (tenant.units as { unit_number?: string })?.unit_number, status: 'active', invited_at: '',
    }));
    for (const property of staff || []) {
      ...
      if (caretaker) users.push({ id: caretaker.id, full_name: caretaker.full_name, email: caretaker.email, phone: caretaker.phone, role: 'caretaker', property_id: property.caretaker_id, property_name: propertyMap.get(property.caretaker_id) || '', unit_number: undefined, status: 'active', invited_at: '' });
    }
